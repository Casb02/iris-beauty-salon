---
id: e0d7d8d2-1080-4a75-9bc7-e31e7086af14
blueprint: page
title: 'Iris Beauty Salon'
template: home
updated_at: 1773246625
hero_eyebrow: 'Welkom bij Iris Beauty Salon'
hero_title_line_one: Ontdek
hero_title_line_two: 'je ware'
hero_title_line_three: schoonheid
hero_intro: 'Een moment van ontspanning en verjonging aan de rand van Hengelo. Wij geloven in natuurlijke schoonheid en persoonlijke aandacht.'
hero_primary_cta_text: 'Afspraak inplannen'
hero_primary_cta_url: '#contact'
hero_secondary_cta_text: Diensten
hero_secondary_cta_url: '#diensten'
hero_decorative_letter: B
intro_label: About
intro_title_lead: 'Waar ontspanning en schoonheid'
intro_title_emphasis: samenkomen
intro_body: 'Stap binnen in een wereld van rust en verfijning. Bij Iris Beauty Salon draait alles om jouw welzijn. Of je nu komt voor een behandeling of gewoon even wilt ontsnappen aan de drukte van alledag, wij zorgen voor een ervaring die je niet snel vergeet.'
services_section_id: diensten
services_label: Services
services_title: Behandelingen
services_cta_text: 'Zie alle diensten'
services_cta_url: '#diensten'
services:
  -
    id: lDsVHWA8
    name: Tandkristallen
    description: 'Voeg een subtiele sparkle toe aan je glimlach met onze hoogwaardige tandkristallen.'
  -
    id: XZ6nNxEt
    name: Tandenbleken
    description: 'Een stralend witte glimlach dankzij onze professionele bleekbehandelingen. Volledig peroxide vrij!'
  -
    id: NYjG4qX6
    name: Browlamination
    description: 'Perfecte, volle wenkbrauwen die de hele dag op hun plek blijven.'
  -
    id: hX2BxDEE
    name: Browstyling
    description: 'Professioneel gevormde wenkbrauwen die perfect bij jouw gezicht passen.'
  -
    id: SzLZtKSz
    name: Lashlift
    description: 'Natuurlijk gelifte wimpers voor een open, frisse blik zonder mascara.'
about_section_id: over
about_label: 'Over Iris'
about_title_lead: 'Persoonlijke aandacht in een'
about_title_emphasis: huiselijke
about_title_tail: sfeer
about_body_left: 'Welkom bij Iris Beauty Salon, waar schoonheid en gezelligheid samenkomen. In mijn salon staat persoonlijke aandacht centraal. Of je nu jong bent of wat ouder, iedereen is welkom om te genieten van een ontspannende behandeling in een rustige omgeving.'
about_body_right: 'Ik geloof dat iedereen mooi is op zijn of haar eigen manier. Mijn doel is om jouw natuurlijke schoonheid naar boven te halen en je een geweldig gevoel te geven.'
about_signature: Iris
about_pillars:
  -
    id: IdkhuBDU
    label: Persoonlijk
  -
    id: vWe25SSd
    label: Rustig
  -
    id: VcI3G4HZ
    label: Vakkundig
fotos_label: "Foto's"
fotos_title: Sfeerimpressie
reviews_label: Testimonials
reviews_title: Ervaringen
reviews_embed_url: 'https://iris-beauty-salon.salonized.com/reviews?layout=embed'
contact_section_id: contact
contact_label: Contact
contact_title: 'Laten we kennismaken'
contact_intro: 'Heb je vragen of wil je direct een afspraak maken? Neem gerust contact op. Ik help je graag verder!'
contact_address: 'Wegtersweg 80-84, 7556BR Hengelo (OV), Overijssel'
contact_phone: '0618127760'
contact_phone_label: '06 18127760'
contact_email: info@irisbeautysalon.nl
contact_form_title: 'Stuur een bericht'
contact_submit_text: 'Verstuur bericht'
footer_copyright: 'Iris Beauty Salon. Alle rechten voorbehouden.'
footer_address: 'Wegtersweg 80-84, 7556BR Hengelo (OV)'
updated_by: 3a49ac04-07d5-4c02-bea7-7c58f55b30bd
---
